...
const Transaction = () => {
  ...
  const generateId = (name, index) => {
    ...
    const nextNumber = String(index + 1).padStart(4, "0");

    return `${formattedInitials}${nextNumber}`;
  };
  ...
  const handleSaveEdit = () => {
    const updatedTransactions = transactions.map((t, index) => {
      if (t.id === editingTransaction.id) {
        return {
          ...editingTransaction,
          id: generateId(editingTransaction.name, index),
        };
      }
      return t;
    });
  ...
  const handleAddTransaction = () => {
    setTransactions([
      ...transactions,
      {
        id: generateId(newTransaction.name, transactions.length),
        ...newTransaction,
      },
    ]);
        ...
        <div className="modal">
          <div className="modalContent">
            ...
            <div className="priority">
              ...
              <button
                className={newTransaction.priorityList === "High" ? "priorityButtonActive" : "priorityButton"}
                onClick={() => handlePriorityChange("High")}
              >
                High
              </button>
              <button
                className={newTransaction.priorityList === "Low" ? "priorityButtonActive" : "priorityButton"}
        ...
        <div className="modal">
          <div className="modalContent">
            ...
            <div className="priority">
              <label>Priority Status:</label>
              <button
                className={editingTransaction.priorityList === "High" ? "priorityButtonActive" : "priorityButton"}
                onClick={() => handlePriorityChange("High")}
              >
                High
              </button>
              <button
                className={editingTransaction.priorityList === "Low" ? "priorityButtonActive" : "priorityButton"}
                onClick={() => handlePriorityChange("Low")}
              >
                Low
              </button>
